...
describe('v4', async () => {
  ...
  function runTest({ name, lang = 'html', text, position, expected }: CompletionFixture) {
    test(name, async () => {
      ...
      expected ??= ({ textEdit }) => ({
        ...
        items: expect.arrayContaining([
          ...
          expect.objectContaining({ label: 'sm:', textEdit }),

          // Static utilities
          expect.objectContaining({ label: 'uppercase', textEdit }),

          // Theme-driven utilities
          expect.objectContaining({ label: 'w-sm', textEdit }),
        ]),
      })

      expect(list).toEqual(expected({ position, textEdit }))
    })
  }
