...
type WalkActionResult = {
  Continue: { kind: WalkKind.Continue }
  Skip: { kind: WalkKind.Skip }
  Stop: { kind: WalkKind.Stop }
  Replace: <T>(nodes: T | T[]) => { kind: WalkKind.Replace; nodes: T[] }
  ReplaceSkip: <T>(nodes: T | T[]) => { kind: WalkKind.ReplaceSkip; nodes: T[] }
  ReplaceStop: <T>(nodes: T | T[]) => { kind: WalkKind.ReplaceStop; nodes: T[] }
}

export const WalkAction: WalkActionResult = {
...
export function walk<T extends object>(
  ast: T[],
  hooks:
    | ((node: T, ctx: VisitContext<T>) => EnterResult<T>) // Old API, enter only
    | {
        enter?: (node: T, ctx: VisitContext<T>) => EnterResult<T>
        exit?: (node: T, ctx: VisitContext<T>) => ExitResult<T>
      },
): void {
...
function walkImplementation<T extends { nodes?: T[] }>(
  ast: T[],
  enter: (node: T, ctx: VisitContext<T>) => EnterResult<T> = () => WalkAction.Continue,
  exit: (node: T, ctx: VisitContext<T>) => ExitResult<T> = () => WalkAction.Continue,
) {
