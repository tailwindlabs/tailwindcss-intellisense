...
/// Look for tagged template literals in the input
///
/// This looks for the " identifier`…` " pattern in the input, which is used to
/// denote tagged template literals in JavaScript.
///
/// We do this by looking for every occurrence of the ` character and then
/// scanning backwards to read the identifier.
pub struct PointsOfInterest<'a> {
  /// The input data
  input: &'a [u8],

  /// A bitmask noting every byte in the current chunk that is a backtick
  matches: u64,
